...
; =============================================================================
; SCRIBE - a fork of apps/word/word.asm (SPEC.md 86).
;
; IT KEEPS THE wd_ PREFIX AND THE wd*.inc FILENAMES, DELIBERATELY. Every
; symbol, every include and every line number is the one word.asm has, so
; `diff -r apps/word apps/scribe` is the whole of what this fork changes and
; an upstream fix can be read straight across. A mechanical sc_ rename over
; 20,000 lines would buy nothing the assembler cares about and would cost
; exactly the thing a long-lived fork needs most - a readable diff against
; what it forked from. NASM resolves the includes out of apps/scribe/ because
; that is the -I on this package's own rule.
;
...
; =============================================================================
    ...
    OS88_HEADER 'SCRIBE', wd_entry, 1  ; bit 0 icon. NOT bit 1: see below
...
WDA_PICT     equ 27             ; Insert > Picture... (SPEC.md 92.9)
WDA_MAX      equ 27
...
WD_PICKB     equ 40             ; the decoded picture's TRANSIENT claim
                                ; (SPEC.md 92.9). 40KB holds a 640x128 or a
...
wd_entry:
    ...
    mov [wd_ovdrv], bl              ; - it is where SCRIBE.OVL lives, and the
...
%ifdef OS88UI_SBDRAG
    ...
    mov [wd_nodrag], al             ; the kernel's arm is not set until that
...
wd_sbmoved: db 0                ; wd_sbclick's SECOND answer (SPEC.md 27.7.10):
                                ; CF says the bar took the click, this says
                                ; whether the VIEW moved because of it
%ifdef OS88UI_SBDRAG
wd_nodrag:  db 0                ; 0xFF = this kernel has no tracking edge
...
%ifdef OS88UI_SBDRAG
; -----------------------------------------------------------------------------
; wd_ondrag / wd_onup - the thumb gesture's other two edges (SPEC.md 13.10.5)
...
; buttons and its menus are wd_mtrack's, a modal poll inside W_ONCLICK - so
...
;
; wd_bounds first: the block's rect and both counts come from the live record,
...
wd_onup:
    ...
    call os88ui_sbdrop
    jc wd_sbd_out
wd_sbd_go:                      ; FLAT labels: the two entries share one tail,
                                ; and a local one belongs to whichever
                                ; non-local label preceded it (13.10.7.4)
    call wd_scrollto            ; AX = the row the hand is on, SI = the window
...
wd_sbclick:
    ...
    push bx
    push dx
    mov byte [wd_sbmoved], 0        ; ...until something actually moves
    call wd_sbhit
    jc .no
...
.set:
    ...
    pop si
.doset:
    call wd_scrollto                ; CF = 1: an END STOP - it did not move,
    jc .yes                         ; so there is nothing to draw
    mov byte [wd_sbmoved], 1
.yes:
    clc
    jmp short .out
.no:
...
wd_rflush:
    ...
    call wd_picdraw                 ; glyphs for it (SPEC.md 92.9)
    jmp .caret
.notpic:
    cmp word [wd_rcols], 0
    je .caret
...
wd_load:
    ...
    call wd_ovcall
    jc .bad                     ; register new ones (SPEC.md 92.6), so it
...
.live:                              ; clicks that land on them. CF=1 = it was
    ...
    ; already at an end stop, used to reach wd_redraw and pay its whole measure
    ; walk to conclude that no row changed.
    cmp byte [wd_sbmoved], 0
...
; -----------------------------------------------------------------------------
; wd_compttl - compose the window title: 'Scribe - ' + wd_name
...
wd_ftab:
    ...
    dw wd_a_pict                    ; Insert > Picture... (SPEC.md 92.9)

wd_mf_ret:
    ret

; -----------------------------------------------------------------------------
; wd_a_pict - Insert > Picture... (SPEC.md 92.9)
;
...
; Scribe' (OSAPI_ABOUT_SET, registered in wd_entry).
; =============================================================================
...
wd_s_sp4:   db '    ', 0
wd_s_sp3:   db '   ', 0
wd_s_about: db 'Scribe', 0
wd_s_abou2: db 'A word processor for os8088', 0
wd_s_abou3: db 'Forked from WORD (SPEC.md 86)', 0
wd_s_ok:    db 'OK', 0
...
; Insert > Picture (SPEC.md 92.9). One string per IMG_E_* code, indexed by it
...
wd_m_loaded2: db 'Module loaded from SCRIBE.OVL', 0
...
; =============================================================================
; REACHING THE PACKAGE'S OWN VARIABLES FROM INSIDE THE MODULE (SPEC.md 92.8.3)
;
...
; --- the two file formats now live in SCRIBE.OVL (SPEC.md 92.8) --------------
...
WD_OVKB      equ 12             ; the claim SCRIBE.OVL is read into, KB. It
                                ...
                                ; 9KB (SPEC.md 92.8). THE MAKEFILE CHECKS THIS
...
wd_ovneed:
    ...
    mov [es:wd_pkgseg], ax          ; MODULE ITSELF (SPEC.md 92.8.3). ES is
...
wd_ovcall:
    ...
    pushf                           ; REASON AND THIS SAYS IT (SPEC.md 92.8.1).
...
; read (SPEC.md 92.8.3). It is IN THE MODULE and not in the package's bss,
...
; --- the file formats, and the six ways in (SPEC.md 92.8) --------------------
...
; --- the file, the dirty prompt and the title (SPEC.md 68.4) -----------------
    ...
    WDVAR wd_wttl,  32      ; 'Scribe - ' + wd_name + NUL: the title
...
; --- the Utilities commands (wdutil.inc, SPEC.md 68.9) -----------------------
    ...
    WDVAR wd_ovmsg, 2       ; word: a message the MODULE wants said, 0 = none.
                            ...
                            ; this on the way back out (SPEC.md 92.8.1)
...
; memory is.
    ...
    WDVAR wd_dpicrun, 1     ; byte: the attribute run wd_dattr just found is
                            ...
                            ; and not a sprm grpprl (SPEC.md 92.7)
