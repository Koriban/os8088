...
the design record behind it. OPTION B WAS BUILT, MEASURED AND REFUSED —
SPEC.md §5.4.2.5** — because the machine Option A was built for is the one
machine that cannot draw the canvas it gained through `gfx_blit1`:
`kern_small` has the slot and not the body, and a repaint through the
fallback measures 24× the band move (PERFORMANCE.md Set 116). A lean body —
the pen, VGA and two-display arms compiled out — came to **+419 bytes** of
`kern_small` against the ~730 §3 priced, and the owner's decision is that the
small build may stay slower; the bytes and the ratio are written beside the
stub. What the same measurement DID fix is the width cliff §2.3 glossed over:
a picture whose width is off the byte grid (the tree's own OS8088.GIF, 466
wide) took the fallback on `kern_big` as well, until `gfx_blit1` learned to
merge a tail byte under a mask. Read §8 first: **open question 1 is ANSWERED
...
docs/plans/MONO-RECLAIM-PLAN.md's question, not this one).
...
5. ~~**Option B or C for small.**~~ **NEITHER — AND B WAS THEN BUILT, MEASURED
   AND REFUSED (SPEC.md §5.4.2.5).** The paragraph below is how it read when
