...
#
# NOT COMPRESSED, and deliberately plain os88pkg.py rather than $(OS88PKG):
# upstream packs every package EXCEPT the two that carry an overlay, WORD and
# SCRIBE, and SHEET is the third. CHART.OVL itself must stay raw whatever the
...
#
# CHART.OVL RIDES WITH IT IN THIS TREE. Without it ch_ovneed fails quietly,
# SHEET runs chartless, and the 20KB pinned claim the overlay lives in never
# exists - so the compaction this row forces would pack a heap this fork never
...
APPS_TOOLS := $(BUILD)/artful.o88 $(BUILD)/browser.o88 $(BUILD)/calc.o88 \
              $(BUILD)/fontview.o88 $(BUILD)/fractal.o88 \
...
#
# ...AND IN THIS FORK THE FILTER IS BACK, because the figure above is of a
# SHEET that is compressed and this one is not. Upstream packs every package
# except the two with overlays, WORD and SCRIBE; this tree's SHEET is a third
# overlay host, so it ships raw at 51 clusters where upstream's ships at ~40,
# and CHART.OVL beside it is 16 more. MEASURED with upstream's unfiltered
# list: 369 clusters against 354.
#
# Three come off, each for a reason that costs the user least:
#  - MODPLUG, 15: on this geometry BEVERLY.MOD rides the media disk, so this
...
# goes on the disk. Filtering one and not the other is the failure upstream's
# paragraph above describes from the other side; deleting the variables
# instead once emptied GAMES/ off this disk with a green build.
APPS_TOOLS_360 := $(filter-out $(BUILD)/modplug.o88 $(BUILD)/fontview.o88,$(APPS_TOOLS))
APPS_GAMES_360 := $(filter-out $(BUILD)/pacman.o88,$(APPS_GAMES))
