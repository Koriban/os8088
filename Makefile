...
$(BUILD)/ether360.img: $(BUILD)/boot360.bin $(BUILD)/kernel.bin $(DRIVERS) $(SYSAPPS) $(COREAPPS360) $(SYSDOC) $(SYSLOGO) $(FACES360) $(FACELIC) $(BUILD)/system.cfg tools/os88disk.py
	python3 tools/os88disk.py -o $@ --size 360 \
		--boot $(BUILD)/boot360.bin --kernel $(BUILD)/kernel.bin \
		$(DRIVERS) $(SYSAPPSARGS) $(COREAPPSARGS360) $(SYSDOC) $(SYSLOGOARG) $(FACESARG360) \
		$(BUILD)/system.cfg
...
# ...and PAC-MAN comes off the 360KB disk, one list over. It arrived from
# upstream after this geometry's budget was already spent, and this fork's
# SHEET is 51 clusters where upstream's is smaller. Upstream later took
# MODPLUG off the list above, which does free enough for it - MEASURED, with
# every game on: 352 of 354 clusters, TWO FREE. That is not room, it is the
# absence of a margin, and SHEET grows in this tree every week. It stays off
# here at 344 and ten free, and ships on every roomier geometry exactly as
# AUDIO, MODPLUG and FONT VIEWER do.
#
# BOTH LISTS BELOW READ THIS VARIABLE - APPS360 for the dependency list and
# APPSARGS360 for the disk's actual contents. Deleting it silently emptied
# GAMES/ off the disk with a green build and a passing suite, because an
# undefined make variable expands to nothing and os88disk validates what it
# is handed, not what was meant.
